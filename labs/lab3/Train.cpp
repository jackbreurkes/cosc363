...
void display(void)
{
   float lgt0_pos[] = {0.0f, 50.0f, 0.0f, 1.0f};  //light0 position (directly above the origin)
   float lgt1_pos[] = {-10.0f, 14.0f, 0.0f, 1.0f};  //light1 position
   float lgt1_dir[] = {-0.8, -1, 0, 0};  //light1 direction
   ...
   glLightfv(GL_LIGHT0, GL_POSITION, lgt0_pos);   //light position

   floor();
   tracks();

   glPushMatrix();
   glRotatef(0 + train_rot, 0, 1, 0);
   ...
   for (int i = 1; i < 5; i++) {
       glPushMatrix();
       glRotatef(-10.5 * i  + train_rot, 0, 1, 0);
       glTranslatef(0, 1, -120);
       wagon();
       glPopMatrix();
   }
...
void trainMover(int value) {
    train_rot += 1;
    glutPostRedisplay();
    glutTimerFunc(10, trainMover, 0);
}
