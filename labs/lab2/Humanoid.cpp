...
void changeTheta(int value) {
    value = value % 80;
    int normalisedValue = value - 40;
    if (normalisedValue >= -20 && normalisedValue < 20) {
        theta = normalisedValue;
    } else {
        if (normalisedValue < -20) {
            theta = -40 - normalisedValue;
        } else {
            theta = 40 - normalisedValue;
        }
    }
    glutPostRedisplay();
    glutTimerFunc(50, changeTheta, ++value);
}
...
int main(int argc, char** argv)
{
   ...
   glutTimerFunc(50, changeTheta, 0);
   glutMainLoop();
   return 0;
}
