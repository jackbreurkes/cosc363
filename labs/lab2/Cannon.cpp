...
void display()  
{
    ...
    glPushMatrix(); // cannonball
        glTranslatef(38.88 + ballDist, 64, 0);
...
void special(int key, int x, int y)
{
    ...
    else if(key == GLUT_KEY_UP) cam_hgt += 2;
...
void ballLaunch(int value) {
    ballDist += 1.0;
    glutPostRedisplay();
    glutTimerFunc(50, ballLaunch, ++value);
}

void keyboard(unsigned char key, int x, int y) {
    if (key == ' ') glutTimerFunc(50, ballLaunch, 1);
}
